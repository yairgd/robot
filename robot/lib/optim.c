...
void rotate_org_depricated(struct vec3 * point,float Rx, float Ry, float Rz) {
	float cx = cos(Rx);
	float sx = sin(Rx);
	float cy = cos(Ry);
	float sy = sin(Ry);
...
//	double des_xyz [3]= {8.411929, 10.603243, 3.53};
...
void forward_calc1() {
	...
	double des_xyz [3]= {9.411929, 7.603243, 1.539922};
	double grad[4];
	double var_value[4] = {0.1,0.2,0.2,0.2};

	endeffector_cost_function_gradient(des_xyz, var_value, grad);

	struct vec3_list * list =  forward_kinetic(var_value);
while (list) {
		printf ("xyz: %lf, %lf,  %lf\n", list->p.x, list->p.y , list->p.z);
		next=list->next;
		free(list);
